...
theme.memory, theme.mem_signal = awful.widget.watch(theme.scripts .. "get-mem.sh", 2,
    function(widget, stdout)
        ...
        if value >= 0.85 then
            mem_bar:set_value(value)
            mem_bar:set_color(theme.bg_urgent)
        else
            mem_bar:set_value(value)
            mem_bar:set_color(theme.fg_normal)
    end
    end)


local membg = wibox.container.background(mem_bar, theme.bg_widget, gears.shape.rectangle)
--                                             left,   right, top,    bottom
theme.memwidget = wibox.container.margin(membg, dpi(2), dpi(2), dpi(4), dpi(4))
-- }}}
...
theme.space, theme.file_signal = awful.widget.watch(theme.scripts .. "get-space.sh", 2,
    function(widget, stdout)
        ...
        local value = tonumber(stdout)
        if value >= 0.85 then
            file_bar:set_value(value)
            file_bar:set_color(theme.bg_urgent)
        else
            file_bar:set_value(value)
            file_bar:set_color(theme.fg_normal)
    end
    end)


local filebg = wibox.container.background(file_bar, theme.bg_widget, gears.shape.rectangle)
--                                             left,   right, top,    bottom
theme.filewidget = wibox.container.margin(filebg, dpi(2), dpi(2), dpi(4), dpi(4))
-- }}}
